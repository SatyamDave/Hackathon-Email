...
export const PriorityInbox: React.FC = () => {
  ...
  const fetchEmails = async () => {
    ...
    setError(null);
    try {
      // Map mockEmails to match the Email interface
      const mappedEmails: Email[] = mockEmails.map(email => ({
        id: email.id,
      ...
      const sorted = mappedEmails.sort((a, b) => a.subject.localeCompare(b.subject));
      setEmails(sorted);
      setReadStatus({});
    } catch (err) {
